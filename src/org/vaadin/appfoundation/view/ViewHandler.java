...
public class ViewHandler implements TransactionListener,
	...
	public static void activateView(Object viewId, boolean changeUriFramgent,
			Object... params) {
		if (viewId == null) {
			throw new NullPointerException("No viewId defined");
		}

		if (!instance.get().viewMap.containsKey(viewId)) {
			ViewNotRegisteredException e = new ViewNotRegisteredException();
			e.setViewId(viewId);
			throw e;
		}

		if (!instance.get().parentMap.containsKey(viewId)) {
			NoViewParentRegisteredException e = new NoViewParentRegisteredException();
			e.setViewId(viewId);
			throw e;
		}
